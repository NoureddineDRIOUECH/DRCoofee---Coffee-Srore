...
<body>
    ...
    <main>
                        ...
                        foreach ($products as $index => $product) {
                            ...
                            if ($index === 0) {
                                ...
                                if ($order['etat'] === 'En attente la confiramtion') {
                                    echo '<td rowspan="' . count($products) . '"><button style = "margine-bottom : 10px" onclick="confirmOrder(' . $orderID . ')">Confirmer</button> <br><br><br><br><br><br>
                                    <button onclick="supprimerOrder(' . $orderID . ')">Supprimer</button></td>';
                                }
                            }
                            echo '</tr>';
                        }
                ...
                <?php
                    ...
                    if ($historyOrders) {
                        foreach ($historyOrders as $historyOrder) {
                            ...
                            echo '<div>';
                            echo '<p style = "color : #902713;display: inline;" >Date d\'Achat : </p><p style = "display: inline;"> ' . $historyOrder['date'] . '</p>';
                            echo '</div>';
                            $totalAmount = 0;

                            foreach ($historyProducts as $historyProduct) {
                                echo '<div>';
                                echo '<p style = "color : #902713;display: inline;">Nom du Produit : </p><p style = "display: inline;"> ' . $historyProduct['nomProduit'] . '</p>';
                                echo '</div>';
                                echo '<div>';
                                echo '<p style = "color : #902713;display: inline;">Prix : </p><p style = "display: inline;"> ' . $historyProduct['prixProduit'] . ' MAD</p>';
                                echo '</div>';
                                echo '<div>';
                                echo '<p style = "color : #902713;display: inline;">Quantité : </p><p style = "display: inline;"> ' . $historyProduct['Quantite'] . ' Pièces</p>';
                                echo '</div>';
                                $totalAmount += $historyProduct['prixProduit'] * $historyProduct['Quantite'];
                            }
                            echo '<div>';
                            echo '<p style = "color : #902713;display: inline;">Total : </p><p style = "display: inline;"> ' . $totalAmount . ' MAD</p>';
                            echo '</div>';
                            echo '</div>';
                        }
                    } else {
        ...
        <div class="logout-btn">
            <form method="get">
                <button type="submit" style="background-color: #FB3636;" name="logout">Déconnexion</button>
            </form>
        </div>
    </main>
    <?php
